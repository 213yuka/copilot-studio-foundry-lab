...
> CS のオーケストレータ動作は **Generative orchestration (生成型オーケストレーション)** が有効である前提。
...
- **Name (CS 側)**: `FoundryDeepResearch`

### Description (CS の Add an agent フォームに入れる文言)
...
3. Foundry agent 側の `instructions` にも **「定型タスクは CS 側で処理されるはずなので、ここに来た時点で高度な質問のはず」** を書いておくと暴走を防げる
4. 接続後すぐに **CS Analytics の Topic 発火率**と **Foundry portal の Tracing 呼出率**を 1 週間モニタし、想定通り振り分けされているか確認
5. 想定外の振り分けが起きたら Description を磨き込む (再デプロイ不要、CS 側の保存だけで反映)
