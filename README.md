...
> **位置付け**: 公式ドキュメントに基づく調査結果 (`research/`) と、**9 つの移行 / 連携 / 運用シナリオ** (A〜I) に対応する再現用素材 (`demo-assets/`)、横断的なガバナンス / FinOps / Evaluation の補助ドキュメント (`docs/`) をセットで提供します。プロダクション用テンプレートではなく、**社内勉強会・PoC・お客様デモの出発点**として利用してください。
...
├── README.md          ← このファイル
├── .gitignore
├── demo-assets/       ← 9 シナリオ (A〜I) の再現素材一式 (手順書・サンプル コード・OpenAPI など)
├── docs/              ← 横断ドキュメント (Governance / FinOps / Evaluation Playbook)
...
| [`demo-assets/`](demo-assets/) | 9 シナリオ (A〜I) の手順書・コード・設定サンプル | [`README.md`](demo-assets/README.md) / [`00-create-cs-agent.md`](demo-assets/00-create-cs-agent.md) |
| [`docs/`](docs/) | シナリオ横断の補助ドキュメント (Governance / FinOps / Evaluation Playbook) | [`governance.md`](docs/governance.md) / [`cost-finops.md`](docs/cost-finops.md) / [`evaluation-playbook.md`](docs/evaluation-playbook.md) |

## アーキテクチャ全体図

シナリオ A〜F (移行 / 連携の中核) を中心に、G (Foundry → M365 逆方向公開) / H (APIM AI Gateway) / I (Evaluation + Red Teaming) が **上位レイヤー** として被さる関係を示しています。
...
flowchart LR
  ...
  Eval[/"Foundry Evaluation +<br/>AI Red Teaming<br/>(シナリオ I)"/]
  ...
  PA --"G: Agent Application 公開<br/>(EAP)"--> M365
  WA --"G: 〃"--> M365
  HA --"G: 〃"--> M365
  ...
  Eval -.Built-in evaluator + Red Teaming で<br/>品質・安全をゲート化.-> PA
  Eval -.〃.-> WA
  Eval -.〃.-> HA
```
...
シナリオは **4 つのレイヤー** に整理しています。シナリオ A〜D / F は共通の出発点として **Microsoft Copilot Studio に同一の `IT-Helpdesk-Sample` エージェントを構築**します (シナリオ E は既存の Microsoft Copilot Studio エージェントを前提に開始)。
...
| **G** | 公開チャネル拡張 | Microsoft Foundry agent を Microsoft 365 Copilot / Teams へ直接公開 | Early Access Preview |
| **H** | ガバナンス / FinOps | Azure API Management を AI Gateway として被せる | APIM コア GA / Foundry 統合 Preview |
| **I** | 品質・安全ゲート | Microsoft Foundry Built-in evaluators + AI Red Teaming Agent | フレームワーク GA / Agent / Red Teaming Preview |
...
2. [`demo-assets/README.md`](demo-assets/README.md) で 9 シナリオ (A〜I) の概要と選定フローチャートを確認し、目的に合うシナリオを選ぶ。
...
5. (本番化を見据える場合) [`docs/governance.md`](docs/governance.md) / [`docs/cost-finops.md`](docs/cost-finops.md) / [`docs/evaluation-playbook.md`](docs/evaluation-playbook.md) を参照し、ガバナンス / 課金 / 品質ゲートの設計を上乗せする。

> 💡 **段階的移行の推奨パス**: いきなり A〜C で全面移行するのではなく、まず **E (モデル品質を 1 Prompt で検証)** → **D (1 機能だけ agent 委譲で検証)** → 必要に応じ **A/B/C で全面移行** → 本番運用に向けて **G (公開チャネル拡張) / H (ガバナンス・FinOps) / I (品質ゲート)** を被せる、という順で進めると低リスクです。F は D / E と並行して進められます。

## 前提・想定読者

- Microsoft Copilot Studio / Microsoft Foundry いずれかに触れた経験がある開発者・アーキテクト
- 移行や連携の判断材料、または社内デモのシナリオを探している方
- 本番リリース前に **ガバナンス / FinOps / Evaluation** の設計を上乗せしたいプラットフォーム エンジニア / FinOps 担当
